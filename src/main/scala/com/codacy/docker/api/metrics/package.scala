...
package object metrics {
    ...
    Reads { json: JsValue =>
      json
        ...
        .flatMap { langStr =>
          Languages
            .fromName(langStr)
            .fold[JsResult[Language]](JsError(
              s"Could not find language for name $langStr"))(JsSuccess(_))
        }
    },
  ...
  implicit def configurationValueToJsValue(
      configValue: MetricsConfiguration.Value): JsValue = {
    configValue match {
  ...
  implicit lazy val configurationValueFormat
    : Format[MetricsConfiguration.Value] = Format(
    implicitly[Reads[JsValue]].map(MetricsConfiguration.Value),
    Writes(configurationValueToJsValue)
  )

  implicit lazy val configurationOptionsKeyFormat
    : OFormat[MetricsConfiguration.Key] =
    Json.format[MetricsConfiguration.Key]
  implicit lazy val configurationOptionsFormat
      ...
      Reads { json: JsValue =>
        JsSuccess(
          json
            ...
            .fold(
              Map.empty[MetricsConfiguration.Key, MetricsConfiguration.Value]) {
              _.map {
                case (k, v) =>
...
private[this] case class MetricsConfigurationValue(value: JsValue)
    extends AnyVal
    with MetricsConfiguration.Value
