...
object DockerMetricsEnvironment {
  ...
  def getConfiguration(configFilePath: File, sourcePath: File): Try[Option[MetricsConfiguration]] = {
    if (configFilePath.exists) {
      for {
        ...
        Some(cfg.copy(files = cfg.files.map(_.map(file => file.copy(path = (sourcePath / file.path).toString)))))
      }
    } else {
